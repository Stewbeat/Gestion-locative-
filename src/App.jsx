...
import {
  ...
  Coins, Calculator, PieChart, TrendingDown, LogOut, Banknote, ArrowDownUp
} from "lucide-react";
...
export default function App() {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const [s, p, q, d, c, ch, am, pr] = await Promise.all([
      ...
      setCharges(ch); setAmort(am); setPret(pr);
      setLoaded(true);
    })();
  ...
  const treso = useMemo(() => {
    ...
    const months = keys.map((k) => {
      ...
      const net = entrees - sortiesCharges - sortiesPret;
      return { key: k, mois: MONTHS_FR[Number(k.slice(5)) - 1], horsBail, reel,
               entrees, sortiesCharges, sortiesPret, sorties: sortiesCharges + sortiesPret, net,
               futur: k >= nowKey && !reel };
    });
    let cum = 0;
    ...
    const netAnnuel = tEntrees - tCharges - tPret;
    ...
    return { months, tEntrees, tCharges, tPret, netAnnuel, capitalRembourse, pire };
  }, [keys, payments, charges, settings, loyerTotal, pret]);
...
function Tresorerie({ year, treso, fiscal, pret, setPret }) {
  const { months, tEntrees, tCharges, tPret, netAnnuel, capitalRembourse, pire } = treso;
  ...
  return (
    <div className="stack">
      ...
      <section className="grid4">
        ...
        <Stat label="Charges payées" value={eur(tCharges)} sub="décaissements réels" tone="red" />
